...
class TemperatureCount: NumberCount {
    ...
    private func accessNumber(number: Int) -> NSAttributedString {
        ...
        let totalStr = countStr + duStr
        
        let totalFont: UIFont = UIFont(name: LATO_THIN, size: 95.0)!
        let duFont: UIFont = UIFont(name: LATO_THIN, size: 95.0)!
        
        let totalRange: NSRange = totalStr.range()             // 全局的区域
        let countRange: NSRange = countStr.rangeFrom(string: totalStr) // %的区域
        let duRange: NSRange = duStr.rangeFrom(string: totalStr)//// °
        
        return totalStr.createAttributedStringAndConfig(configs: [
            //全局设置
            ConfigAttributedString.font(font: totalFont, range: countRange),
            
            //°数
            ConfigAttributedString.font(font: duFont, range: duRange),
            
            //局部设置
            ConfigAttributedString.foregroundColor(color: UIColor(hexString: "9000ff"), range: totalRange)
            ])
