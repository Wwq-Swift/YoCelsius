...
class HumidityCount: NumberCount {
    ...
    private func accessNumber(number: Int) -> NSAttributedString {
        ...
        let totalStr = countStr + percentStr
        
        let font1: UIFont = UIFont(name: LATO_LIGHT, size: 40.0)!
        let font2: UIFont = UIFont(name: LATO_LIGHT, size: 19.0)!
        ...
        return totalStr.createAttributedStringAndConfig(configs: [
            ...
            //局部设置
            ConfigAttributedString.foregroundColor(color: UIColor(hexString: "C2C2C2"), range: totalRange),
            ConfigAttributedString.foregroundColor(color: UIColor.black, range: countRange)
            ])
